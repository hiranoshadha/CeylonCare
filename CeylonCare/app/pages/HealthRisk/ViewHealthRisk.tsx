...
const ViewHealthRisk = ({ navigation }: any) => {
  ...
  const fetchHealthRisks = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.78.225:5000/riskassessment/user/${userId}`, {
        method: "GET",
        headers: {
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`http://192.168.78.225:5000/riskassessment/${riskId}`, {
                method: "DELETE",
                headers: {
                  "Content-Type": "application/json",
                },
      ...
      >
        <View style={styles.riskHeader}>
          ...
          <View style={styles.riskBadges}>
            <View style={[
              styles.riskBadge, 
              item.diabetesRisk.includes("High") ? styles.highRiskBadge : 
              item.diabetesRisk.includes("Medium") ? styles.mediumRiskBadge : styles.lowRiskBadge
            ]}>
              <Text style={styles.riskBadgeText}>{item.diabetesRisk}</Text>
            </View>
            <View style={[
              styles.riskBadge, 
              item.hypertensionRisk.includes("High") ? styles.highRiskBadge : 
              item.hypertensionRisk.includes("Medium") ? styles.mediumRiskBadge : styles.lowRiskBadge
            ]}>
              <Text style={styles.riskBadgeText}>{item.hypertensionRisk}</Text>
            </View>
          </View>
        </View>

        <View style={styles.riskDetails}>
          <Text style={styles.detailText}>Age: {item.age} years</Text>
          <Text style={styles.detailText}>BMI: {item.bmi}</Text>
          <Text style={styles.detailText}>Gender: {item.gender}</Text>
          <Text style={styles.detailText}>Smoking: {item.smokingStatus}</Text>
        </View>
